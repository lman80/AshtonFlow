...
"""
    ...
    /// When false, screenshots are never captured and Screen Recording permission
    /// is never needed. Context still works from app name, window title, and
    /// selected text. Injected from AppState's user-facing toggle.
    private let screenRecordingEnabled: Bool
...
"""
    ...
    private func captureActiveWindowScreenshot(
        ...
        // Screen recording is turned off, so never capture the screen or request
        // its permission. Context-aware features keep working from the app name,
        // window title, and selected text (read via the Accessibility API).

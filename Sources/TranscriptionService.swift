...
class TranscriptionService {
    ...
    /// Optional explicit timeout (seconds). Used for transcribing dragged-in
    /// files, which can be much longer than a quick dictation.
    private let timeoutOverride: TimeInterval?
    private var transcriptionTimeoutSeconds: TimeInterval {
        if let timeoutOverride, timeoutOverride > 0 {
